...
export default function AISidebar({ content, researchQuery, onResearchComplete, onSpellingIssuesChange, onFactCheckIssuesChange, manualFactCheckTrigger, onHighlightText, ignoredFactChecks = new Set(), onIgnoredFactChecksChange }: AISidebarProps) {
  ...
  const grammarSpellingIssuesRef = useRef<GrammarSpellingIssue[]>([]);

  useEffect(() => {
    grammarSpellingIssuesRef.current = grammarSpellingIssues;
  ...
  useEffect(() => {
    if (grammarCheckEnabled && content) {
      ...
      const lastTextContent = lastCheckedContent.current.replace(/<[^>]*>/g, '').trim();
      const wordCount = textContent.split(/\s+/).filter(word => word.length > 0).length;
      
      if (textContent !== lastTextContent && wordCount >= 2) {
        // Check if any existing issues were fixed in this update
        const currentIssueTexts = grammarSpellingIssuesRef.current.map(issue => issue.text.toLowerCase().trim());
